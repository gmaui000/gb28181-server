...
use crate::net::state::{
    Gate, GateAccept, GateListener, Protocol, Zip, CHANNEL_BUFFER_SIZE, TCP_HANDLE_MAP,
};
...
pub async fn listen(
    ...
    tx: oneshot::Sender<GateListener>,
    ...
    match protocol {
        ...
        Protocol::ALL => {
            ...
            tx.send(gate_listener).map_err(|_err| {
                GlobalError::new_sys_error("net io listen err:channel has drop", |msg| {
                    error!("{msg}")
                })
            })?;
...
pub async fn accept(
    rx: oneshot::Receiver<GateListener>,
    tx: Sender<GateAccept>,
) -> GlobalResult<()> {
    match rx.await {
        Ok(gate_listener) => {
            match gate_listener {
                ...
                GateListener::All((tcp_gate, tcp_listener), (udp_gate, udp_socket)) => {
                    let sender = tx.clone();
                    let local_addr = *tcp_gate.get_local_addr();
                    ...
                    tokio::spawn(async move {
                        loop {
                            ...
                            let gate1 = Gate::new(local_addr, input.clone(), lone_output_rx);
                            let _ =
                                tcp::accept(gate1, &tcp_listener, sender.clone(), lone_output_tx)
                                    .await
                                    .hand_log(|msg| error!("{msg}"));
                        }
                    });
        ...
        Err(e) => Err(GlobalError::new_sys_error(
            &format!("net io: {}", e),
            |msg| error!("{msg}"),
        ))?,
    }
