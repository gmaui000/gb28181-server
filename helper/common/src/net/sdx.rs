use crate::net::state::{Gate, GateListener, Protocol, Zip, CHANNEL_BUFFER_SIZE};
...
#[cfg(feature = "net")]
pub async fn run_by_tokio(
    tu: (Option<TcpListener>, Option<UdpSocket>),
) -> GlobalResult<(Sender<Zip>, Receiver<Zip>)> {
    let (listen_tx, listen_rx) = oneshot::channel();
    //socket 读数据通道 input
    let (input_tx, input_rx) = channel(CHANNEL_BUFFER_SIZE);
    //socket 写数据通道 output
    let (output_tx, output_rx) = channel(CHANNEL_BUFFER_SIZE);
    match tu {
        (Some(tl), None) => {
            let gate = Gate::new(
                tl.local_addr().hand_log(|msg| error!("{msg}"))?,
                input_tx,
                output_rx,
            );
            let listener = tcp::listen_by_std(gate, tl)?;
            listen_tx.send(listener).map_err(|_err| {
                GlobalError::new_sys_error("net io listen err:channel has drop", |msg| {
                    error!("{msg}")
                })
            })?;
        }
        (None, Some(us)) => {
            let gate = Gate::new(
                us.local_addr().hand_log(|msg| error!("{msg}"))?,
                input_tx,
                output_rx,
            );
        ...
        (Some(tl), Some(us)) => {
            let (tw_tx, tw_rx) = channel(CHANNEL_BUFFER_SIZE);
            let tcp_gate = Gate::new(
                tl.local_addr().hand_log(|msg| error!("{msg}"))?,
                input_tx.clone(),
                tw_rx,
            );
            let tcp_listener = tcp::listen_by_std(tcp_gate, tl)?;
            let (uw_tx, uw_rx) = channel(CHANNEL_BUFFER_SIZE);
            let udp_gate = Gate::new(
                us.local_addr().hand_log(|msg| error!("{msg}"))?,
                input_tx.clone(),
                uw_rx,
            );
            ...
            listen_tx.send(gate_listener).map_err(|_err| {
                GlobalError::new_sys_error("net io listen err:channel has drop", |msg| {
                    error!("{msg}")
                })
            })?;
    ...
    let (accept_tx, accept_rx) = channel(CHANNEL_BUFFER_SIZE);
    let _ = crate::net::core::accept(listen_rx, accept_tx)
        .await
