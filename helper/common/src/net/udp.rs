...
use crate::net::state::{
    Association, Gate, GateAccept, GateListener, Package, Protocol, Zip, SOCKET_BUFFER_SIZE,
};
use bytes::Bytes;
...
pub fn listen_by_std(
    gate: Gate,
    std_udp_socket: std::net::UdpSocket,
...
pub async fn read(local_addr: SocketAddr, udp_socket: &UdpSocket, tx: Sender<Zip>) {
    loop {
        ...
        match udp_socket.try_recv_from(&mut buf) {
            ...
            Err(err) => {
                warn!(
                    "【UDP read failure】 【Local_addr = {}】 【err = {:?}】",
                    local_addr.to_string(),
                    err,
                );
                break;
...
pub async fn write(udp_socket: &UdpSocket, mut rx: Receiver<Zip>) {
    while let Some(zip) = rx.recv().await {
        ...
        match zip {
            ...
            Zip::Event(_event) => {
                info!("UDP Events are not supported")
            }
        }
    }
}
