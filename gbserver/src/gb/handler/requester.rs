    ...
    if req.method == Method::Register {
        ...
        match State::check_session(tx.clone(), bill, &device_id).await? {
            ...
            State::Expired => {
                ...
                let zip =
                    Zip::build_data(Package::new(bill.clone(), Bytes::from(unregister_response)));
...
impl State {
    ...
    async fn check_session(
        ...
        if RWSession::get_device_id_by_association(bill).is_some()
            || RWSession::has_session_by_device_id(device_id)
        {
            Ok(State::Usable)
        } else {
                ...
                Some((heart, enable, expire, reg_ts, on)) => {
                    if enable == 0 {
                        ...
                        if reg_ts + Duration::seconds(expire as i64) > Local::now().naive_local() {
                            ...
                            //如果设备是离线状态，则更新为在线
                            if on == 0 {
                                GbsDevice::update_gbs_device_status_by_device_id(device_id, 1)
                                    .await?;
                            }
                            Ok(State::ReCache)
                        } else {
...
impl Register {
    ...
    async fn login_ok(
        ...
        let gbs_device = GbsDevice::build_gbs_device(req)?;
        gbs_device.insert_single_gbs_device_by_register().await?;
    ...
    async fn logout_ok(
        ...
        let zip = Zip::build_data(Package::new(bill.clone(), Bytes::from(ok_response)));
        let _ = tx.clone().send(zip).await.hand_log(|msg| warn!("{msg}"));
        GbsDevice::update_gbs_device_status_by_device_id(device_id, 0).await?;
    ...
    async fn unauthorized(req: &Request, tx: Sender<Zip>, bill: &Association) -> GlobalResult<()> {
        ...
        let zip = Zip::build_data(Package::new(
            bill.clone(),
            Bytes::from(unauthorized_register_response),
        ));
        let _ = tx.clone().send(zip).await.hand_log(|msg| error!("{msg}"));
        Ok(())
    }
...
impl Message {
    async fn process(
        ...
        match parse_xlm_to_vec(&req.body) {
            Ok(vs) => {
                let response =
                    ResponseBuilder::build_register_ok_response(&req, bill.get_remote_addr())?;
                for (k, v) in &vs {
                    if MESSAGE_TYPE.contains(&&k[..]) {
                        ...
                        let zip =
                            Zip::build_data(Package::new(bill.clone(), Bytes::from(response)));
                        let _ = tx.clone().send(zip).await.hand_log(|msg| error!("{msg}"));
                        break;
                    }
                }
    ...
    async fn device_info(vs: Vec<(String, String)>) {
        let _ = GbsDeviceExt::update_gbs_device_ext_info(vs)
            .await
            ...
            Ok(vs) => {
                ...
                for (k, v) in &vs {
                    if MESSAGE_TYPE.contains(&&**k) {
                        match &v[..] {
                            MESSAGE_NOTIFY_CATALOG => {
                                GbsDeviceChannel::insert_gbs_device_channel(device_id, vs).await?;
                            }
                            _ => {
...
mod tests {
    ...
    use common::utils::crypto::{default_decrypt, default_encrypt};

    #[test]
    fn test_decrypt() {
        let pass = default_encrypt("admin@123").expect("pass en error");
        println!("pass_en = {}", pass);
        println!(
            "pass_de = {}",
            default_decrypt(&pass).expect("pass de error")
        );
    }
