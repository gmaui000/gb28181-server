...
use crate::gb::handler::events::event::EventSession;
...
pub async fn read(mut input: Receiver<Zip>, output_tx: Sender<Zip>) {
    while let Some(zip) = input.recv().await {
        match zip {
            Zip::Data(Package { association, data }) => match SipMessage::try_from(data) {
                Ok(msg) => match msg {
                    SipMessage::Request(req) => {
                        info!(
                            ...
                            &req.version,
                            &req.headers,
                            GB18030.decode(&req.body).0
                        );
                        let _ =
                            handler::requester::hand_request(req, output_tx.clone(), &association)
                                .await;
                    }
